        ...
        for mode in (TEST_MODE,):
            ...
            for config in regression_tests[compiler]:
                for reg_path, test_path in regression_tests[compiler][config]:
                    ...
                    if use_override:
                        local_override_path = os.path.join(test.runpath, override_fname)
            ...
            finished_tests = {test: False for test in running_tests}

            while(any(test.process.poll() is None for test in running_tests)):
                for test in running_tests:
                    # Skip any finished tests
                    if finished_tests[test]:
                        continue

                    if test.process.poll() is not None:
                        print('{}: Test complete!'.format(test.refpath))
                        finished_tests[test] = True
