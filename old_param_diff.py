...
def report_param_diff():
    # TODO: Validate paths
    diff_left, diff_right = sys.argv[1:3]
    ...
    # Generate a global sets of parameters across expts for each parameter file
    # NOTE: Potentially problematic; consider per-experiment hashing
    for lwalk, rwalk in zip(os.walk(diff_left), os.walk(diff_right)):
        lpath, ldirs, lfiles = lwalk
        rpath, rdirs, rfiles = rwalk

        # Is this always true?  How does git difftool handle add/rm?  TODO
        assert(lfiles == rfiles)

        for lfile, rfile in zip(lfiles, rfiles):
            # Skip non-parameter files
            if not lfile.startswith('MOM_parameter_doc.'):
                continue

            pfile = lfile.split('.')[1]

            lpar = parse_param(os.path.join(lpath, lfile))
            rpar = parse_param(os.path.join(rpath, rfile))
